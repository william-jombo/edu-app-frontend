...
const studentAnnouncementsAPI = {
  getAnnouncements:       async ()      => get('/api/students/announcements.php?action=list').then(r => r.json()),
  getUnreadCount:         async ()      => get('/api/students/announcements.php?action=unread_count').then(r => r.json()),
  getRecentAnnouncements: async (n = 1) => get(`/api/students/announcements.php?action=recent&limit=${n}`).then(r => r.json()),
  markAsRead:             async (id)    => post('/api/students/announcements.php?action=mark_read', { announcement_id: id }).then(r => r.json()),
  markAllAsRead:          async ()      => post('/api/students/announcements.php?action=mark_all_read', {}).then(r => r.json()),
};
...
export const StudentNotificationBell = () => {
  ...
  const fetchUnreadCount = async () => {
    try {
      const r = await studentAnnouncementsAPI.getUnreadCount();
  ...
  const fetchAnnouncements = async () => {
    setLoading(true);
    try {
      const r = await studentAnnouncementsAPI.getAnnouncements();
      if (r.success) {
        setAnnouncements(
          r.data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
        );
      }
    } catch {}
  ...
  const handleMarkAsRead = async (id) => {
    try {
      const r = await studentAnnouncementsAPI.markAsRead(id);
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const r = await studentAnnouncementsAPI.markAllAsRead();
      if (r.success) { fetchUnreadCount(); fetchAnnouncements(); }
    } catch {}
  };
  ...
  const closeAll = () => { setShowPanel(false); setSelected(null); };
  ...
  return (
    <>
      ...
      {showPanel && (
        ...
        >
          <div className="bg-white w-full sm:w-96 sm:h-full h-[88vh] rounded-t-2xl sm:rounded-none sm:rounded-l-2xl flex flex-col shadow-2xl overflow-hidden">
            ...
            {!selected ? (
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-0.5">
                                  ...
                                  {!a.is_read && <span className="w-2 h-2 rounded-full bg-indigo-500 flex-shrink-0" />}
                                </div>
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {(() => {
                    const pr = getPriority(selected.priority);
                    const Icon = pr.icon;
                    return (
                      <>
                        <div className="rounded-2xl border border-slate-100 overflow-hidden">
                          ...
                          <div className="p-4">
                            <div className="flex items-center gap-2 mb-2">
                              <Icon className={`w-4 h-4 ${pr.iconCls}`} />
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${pr.badge}`}>
                                {pr.label} Priority
                              </span>
                            </div>
                            ...
                            <p className="text-xs text-slate-400 flex items-center gap-1">
                              ...
                              {selected.formatted_date || formatDate(selected.created_at)}
                            </p>
                          </div>
                        </div>
...
export const StudentAnnouncementsWidget = () => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const r = await studentAnnouncementsAPI.getRecentAnnouncements(3);
        if (r.success) {
      ...
      <div className="px-4 pt-4 pb-3 flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 leading-tight">From your school</p>
          </div>
        </div>
